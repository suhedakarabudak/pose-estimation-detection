...
if up_file and uploaded:
    ...
    try:
        ...
        while vf.isOpened():
            ...
            # Kareyi FastAPI'ye gönder
            _, img_encoded = cv2.imencode('.jpg', frame)
            try:
                response = requests.post("http://localhost:8000/predict_frame/", files={"file": img_encoded.tobytes()})
                response.raise_for_status()
                if response.status_code == 200:
                    predictions = response.json().get("predictions", [])
                    # Tahminleri kare üzerinde işaretle
                    cv2.putText(out_frame, f"Predictions: {predictions}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
                else:
